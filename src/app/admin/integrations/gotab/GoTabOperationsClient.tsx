...
function KdsPreview({ item, onAction }: { item: StoredEventFoodActivity; onAction: (action: "HOLD" | "SEND_NOW" | "RETRY" | "CANCEL") => Promise<void> }) {
  ...
  return <article className="gotab-kds-preview"><header><span>DRY-RUN KDS PREVIEW</span><strong>{item.event_name}</strong></header><dl><div><dt>Request type</dt><dd>{label}</dd></div><div><dt>Product</dt><dd>{item.display_name}</dd></div><div><dt>Pan size</dt><dd>{item.pan_size.replaceAll("_", " ")}</dd></div><div><dt>Quantity</dt><dd>{item.quantity}</dd></div><div><dt>Station</dt><dd>{item.preparation_station}</dd></div><div><dt>Food service</dt><dd>{formatDate(item.food_service_time)}</dd></div><div><dt>Food Runner</dt><dd>{item.requester_name || "Not entered"}</dd></div><div><dt>Status</dt><dd>{item.dispatch_status.replaceAll("_", " ")}</dd></div><div><dt>GoTab order</dt><dd>{item.gotab_order_uuid || "Not sent"}</dd></div></dl><p>No live GoTab order is created from this preview.</p><footer><button className="button-link" onClick={() => void onAction("SEND_NOW")} type="button">Send now</button><button className="button-link" onClick={() => void onAction("HOLD")} type="button">Hold</button>{item.dispatch_status === "FAILED" ? <button className="button-link" onClick={() => void onAction("RETRY")} type="button">Retry</button> : null}<button className="button-link" onClick={() => void onAction("CANCEL")} type="button">Cancel pending</button></footer></article>;
}
